...
4. Запустить проект одной командой:
   ...
   ```
   Поднимутся три контейнера — `db` (PostgreSQL), `backend` (Django, порт 8000), `frontend` (Vite, порт 5173). Миграции и демо-данные (пользователи, справочник услуг) применяются автоматически. Дождаться в логе строк `Starting development server...` и `VITE ready`.
